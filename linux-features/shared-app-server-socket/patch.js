...
function sharedTransportClassSource(symbols) {
  return (
    ...
    "kind=`websocket`;proxyStreams=new Set;authority=null;authorityError=null;authorityReady=null;lockIdentity=null;ownerStartTime=null;socketIdentity=null;adoptedAuthority=!1;disposed=!1;" +
    "constructor(e,t){if(typeof t!==`function`)throw Error(`shared app-server socket requires a config override callback`);this.socketPath=e;this.lockPath=`${e}.lock`;this.privateFallbackSocketPath=process.env.CODEX_LINUX_APP_SERVER_PRIVATE_FALLBACK_SOCKET??null;this.getConfigOverrides=t}" +
    "supportsReconnect(){return!0}" +
    "authorityArgs(){let e=[`app-server`],t=process.env.CODEX_LINUX_APP_DIR;if(t&&require(`node:path`).isAbsolute(t))try{let n=require(`node:path`).join(t,`.codex-linux`,`desktop-app-server-remote-control-enabled`),r=require(`node:fs`).lstatSync(n);!r.isSymbolicLink()&&r.isFile()&&require(`node:fs`).readFileSync(n,`utf8`)===`version=1\nowner=desktop\n`&&e.push(`--remote-control`)}catch(e){if(e?.code!==`ENOENT`)throw e}return e.push(`--listen`,`unix://${this.socketPath}`),e}" +
    "async adoptCanonicalAuthority(){if(process.env.CODEX_LINUX_ADOPT_CANONICAL_APP_SERVER!==`1`)return!1;let e=require(`node:fs`),t=require(`node:path`),n=process.env.CODEX_HOME??(process.env.HOME?t.join(process.env.HOME,`.codex`):null);if(!n)return!1;let r=t.join(n,`app-server-control`,`app-server-control.sock`);if(t.resolve(this.socketPath)!==t.resolve(r))return!1;try{let n=e.lstatSync(r),o=e.lstatSync(t.dirname(r)),i=typeof process.getuid==`function`?process.getuid():n.uid;if(n.isSymbolicLink()||!n.isSocket()||n.uid!==i||(n.mode&511)!==384||o.isSymbolicLink()||!o.isDirectory()||o.uid!==i||(o.mode&511)!==448||!await this.socketPathIsLive())return!1;return this.adoptedAuthority=!0,!0}catch(e){if(e?.code===`ENOENT`)return!1;throw e}}" +
