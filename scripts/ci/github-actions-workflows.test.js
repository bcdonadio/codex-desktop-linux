...
test("official Linux validation runs fully on every pull request but not hourly", () => {
  ...
  assert.doesNotMatch(workflow, /npx --yes @electron\/asar(?:\s|$)/);
  assert.equal(
    workflow.match(/npx --yes @electron\/asar@4\.3\.0 extract/g)?.length,
    2,
    "every workflow ASAR extraction must use the reviewed exact package version",
  );
  assert.match(
    workflow,
    /ELECTRON_ASAR_INTEGRITY: sha512-k\/FFC\/NQoTykGBi\/Ga4L4KorsAKDdkK0LfNVG90eUG6vPVpJaL3iR9V1ZLbzBAF3QpojK5DcaGOdQOheKZv4JQ==/,
  );
  assert.match(workflow, /test "\$actual_integrity" = "\$ELECTRON_ASAR_INTEGRITY"/);
...
test("install-deps workflow covers apt and pacman Rust bootstrap", () => {
  ...
  assert.match(
    pacman,
    /docker\.io\/library\/archlinux:base-devel@sha256:fdff15f24df062598faebf380430955a9bd2109736e179ebb354f1208f725774/,
  );
